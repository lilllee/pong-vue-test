...
<script setup>
  ...
  import {getStompSocket, initializeSocket} from "@/service/StompSocket";
  ...
  onMounted(() => {
    ...
    /** connect socket */
    const nickname = localStorage.getItem("nickname") || "defaultNickname";
    initializeSocket(nickname);
    socket.value = getStompSocket();
    if (!socket.value) {
